...
<div id="custom-format-settings" class="bg-white rounded-xl shadow-md p-4 sm:p-6 border border-gray-200 hidden">
    <h3 class="text-sm sm:text-md font-semibold text-gray-700 mb-3 sm:mb-4 flex items-center">
        ...
        {{ __('debates_format.configure_custom_format') }}
    </h3>
    ...
    <div class="mb-4 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 border-l-4 border-yellow-400 rounded-md">
        <div class="flex">
            ...
            <div class="ml-3">
                <h4 class="text-sm font-medium text-yellow-800 mb-2">{{ __('debates_format.custom_format_guide_title') }}</h4>
                <p class="text-xs text-yellow-700 mb-2">{{ __('debates_format.custom_format_guide') }}</p>
                <ul class="list-disc list-inside text-xs text-yellow-700 space-y-1">
                    <li>{{ __('debates_format.custom_format_tip1') }}</li>
                    <li>{{ __('debates_format.custom_format_tip2') }}</li>
                    <li>{{ __('debates_format.custom_format_tip3') }}</li>
                </ul>
            </div>
        </div>
    </div>
        ...
        <div class="turn-card border rounded-lg p-3 sm:p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
            <div class="flex justify-between items-center mb-2 sm:mb-3">
                <div class="flex items-center">
                    ...
                    <h4 class="turn-title text-xs sm:text-sm font-medium ml-2 text-gray-700">{{ __('debates_format.part') }} 1</h4>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 sm:gap-4">
                <div class="sm:col-span-3">
                    <label class="block text-xs text-gray-500">{{ __('rooms.side') }}</label>
                    <select name="turns[0][speaker]"
                        ...
                        <option value="affirmative" {{ old('turns.0.speaker') == 'affirmative' ? 'selected' : '' }}>{{ __('rooms.affirmative_side') }}</option>
                        <option value="negative" {{ old('turns.0.speaker') == 'negative' ? 'selected' : '' }}>{{ __('rooms.negative_side') }}</option>
                    </select>
                </div>
                <div class="sm:col-span-5">
                    <label class="block text-xs text-gray-500">{{ __('debates_format.part_name') }}</label>
                    <input type="text" name="turns[0][name]" value="{{ old('turns.0.name') }}"
                        placeholder="{{ __('debates_format.placeholder_part_name') }}"
                    ...
                    <datalist id="part-suggestions">
                        <option value="{{ __('debates_format.suggestion_constructive') }}">
                        <option value="{{ __('debates_format.suggestion_first_constructive') }}">
                        <option value="{{ __('debates_format.suggestion_second_constructive') }}">
                        <option value="{{ __('debates_format.suggestion_rebuttal') }}">
                        <option value="{{ __('debates_format.suggestion_first_rebuttal') }}">
                        <option value="{{ __('debates_format.suggestion_second_rebuttal') }}">
                        <option value="{{ __('debates_format.suggestion_questioning') }}">
                        <option value="{{ __('debates_format.suggestion_prep_time') }}">
                    </datalist>
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-xs text-gray-500">{{ __('debates_format.duration_minutes') }}</label>
                ...
                <div class="sm:col-span-2 flex flex-col justify-end">
                    <div class="flex items-center space-x-2">
                        <label class="inline-flex items-center">
                            ...
                            <span class="ml-1 text-xs text-gray-500">{{ __('debates_format.question_time') }}</span>
                        </label>
                        <label class="inline-flex items-center">
                            ...
                            <span class="ml-1 text-xs text-gray-500">{{ __('debates_format.prep_time') }}</span>
                        </label>
                    </div>
                </div>
            </div>
