...
<div id="free-format-settings" class="bg-white rounded-xl shadow-md p-4 sm:p-6 border border-gray-200 hidden">
    <h3 class="text-sm sm:text-md font-semibold text-gray-700 mb-3 sm:mb-4 flex items-center">
        ...
        {{ __('debates_format.free_format_settings') }}
    </h3>
    ...
    <div class="mb-4 p-4 bg-gradient-to-r from-purple-50 to-indigo-50 border-l-4 border-purple-400 rounded-md">
        <div class="flex">
            ...
            <div class="ml-3">
                <h4 class="text-sm font-medium text-purple-800 mb-2">{{ __('debates_format.free_format_benefits') }}</h4>
                <p class="text-xs text-purple-700">{{ __('debates_format.free_format_description') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div>
            <label for="turn_duration" class="block text-xs sm:text-sm font-medium text-gray-700 mb-2 flex items-center">
                {{ __('debates_format.turn_duration') }}
            ...
            <div class="relative">
                ...
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    <span class="text-gray-500 text-xs sm:text-sm">{{ __('debates_format.minute_unit') }}</span>
                </div>
            </div>
            @if($errors)
        ...
        <div>
            ...
            <div class="relative">
                ...
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    <span class="text-gray-500 text-xs sm:text-sm">{{ __('debates_format.turn_unit') }}</span>
                </div>
            </div>
            <p class="mt-1 text-xs text-gray-500">{{ __('forms.even_numbers_only') }}</p>
            @if($errors)
